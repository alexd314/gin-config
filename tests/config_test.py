...
@config.configurable
def required_with_vkwargs(arg1, arg2, arg3, kwarg1=4, kwarg2=5, kwarg3=6, **kwargs):
...
NamedTuple = collections.namedtuple("ConfigurableNamedTuple", ["field1", "field2"])
...
class ConfigurableExternalSubclass(configurable_external_class):
    ...
    def __init__(self, kwarg1=None, kwarg2=None, kwarg3=None):
        super(ConfigurableExternalSubclass, self).__init__(kwarg1=kwarg1, kwarg2=kwarg2)
        self.kwarg3 = kwarg3
...
class ConfigTest(absltest.TestCase):
    ...
    """
        include_path = os.path.join(
            absltest.get_default_test_srcdir(), "gin/testdata/my_other_func.gin"
        )
    ...
    def testInvalidIncludeError(self):
        config_file = os.path.join(
            absltest.get_default_test_srcdir(), "gin/testdata/invalid_include.gin"
        )
        err_msg_regex = (
            "Unable to open file: not/a/valid/file.gin. " "Searched config paths:"
        )
    ...
    """
        expected_msg = (
            r"No configurable matching 'WhatAmI'\.\n"
    ...
    """
        expected_err_msg = (
            "No configurable matching 'unknown'.\n" "  In bindings string line 3"
        )
        ...
        with absltest.mock.patch.object(logging, "info") as mock_log:
            ...
            for log in mock_log.call_args_list:
                ...
                if "not.a.real.module" in log:
                    if "Traceback" in log:
                        self.fail(
                            "Traceback included for non-nested unknown import log."
                        )
                    else:
                        found_log = True
                        break
    ...
    """
        expected_err_msg = r"No configurable matching reference '@UnknownReference\(\)'"
        with self.assertRaisesRegex(ValueError, expected_err_msg):
            config.parse_config(config_str)
        with self.assertRaisesRegex(ValueError, expected_err_msg):
            config.parse_config(config_str, skip_unknown=["moose"])

        config.parse_config(config_str, skip_unknown=["UnknownReference", "unknown"])
    ...
    """
        ...
        self.assertEqual(
            instance.__dict__, pickle.loads(pickle.dumps(instance)).__dict__
        )
    ...
    """
        ...
        with self.assertRaises(ValidateHookError):
            ConfigurableClassWithMetaAndNew(
                post_new_hook=fail
    ...
    """
        ...
        instance = (
            ConfigurableClassWithMetaAndNew(  # pylint: disable=unexpected-keyword-arg
    ...
    """
        ...
        self.assertEqual(
            sub_instance.__dict__, pickle.loads(pickle.dumps(sub_instance)).__dict__
        )
    ...
    """
        ...
        self.assertEqual(
            instance.kwarg1, "statler"
        )  # pytype: disable=attribute-error  # kwargs-checking
        self.assertEqual(
            instance.kwarg2, "waldorf"
        )  # pytype: disable=attribute-error  # kwargs-checking
    ...
    """
        ...
        self.assertEqual(
            super_instance.kwarg1, "one"
        )  # pytype: disable=attribute-error  # kwargs-checking
        self.assertIsNone(
            super_instance.kwarg2
        )  # pytype: disable=attribute-error  # kwargs-checking
        ...
        self.assertEqual(
            sub_instance.kwarg2, "two"
        )  # pytype: disable=attribute-error  # kwargs-checking
        self.assertEqual(
            sub_instance.kwarg3, "three"
        )  # pytype: disable=attribute-error  # kwargs-checking
    ...
    """
        ...
        self.assertEqual(
            config._OPERATIVE_CONFIG["", selector],
            {"kwarg1": "sub_kwarg1", "kwarg2": None, "kwarg3": None},
        )
    ...
    def testRequiredArgs(self):
        ...
        with config.config_scope("scope"):
            self.assertEqual(
                required_args(config.REQUIRED, config.REQUIRED, 3), (1, 2, 3, 4, 5, 6)
            )
    ...
    def testRequiredArgsAndKwargs(self):
        with self.assertRaisesRegex(
            RuntimeError, "arg2.*kwarg1.*kwarg2|arg2.*kwarg2.*kwarg1"
        ):
            required_args(
    ...
    def testRequiredArgsVkwargs(self):
        with self.assertRaisesRegex(
            RuntimeError, "arg2.*kwarg1.*kwarg6|arg2.*kwarg6.*kwarg1"
        ):
    ...
    """
        ...
        self.assertIn(
            "fn_with_kw_only_args.kwarg1 = None", config.operative_config_str()
        )
        ...
        self.assertIn(
            "fn_with_kw_only_args.kwarg1 = 'kwarg1'", config.operative_config_str()
        )
    ...
    def testPrintAndReturnNestedIncludesAndImports(self):
        ...
        self.assertListEqual(result.imports, ["gin.testdata.import_test_configurables"])
    ...
    """
        ...
        self.assertDictEqual(
            config.get_bindings("configurable1"),
            {
                "non_kwarg": "kwarg1",
                "kwarg2": 123,
            },
        ...
        self.assertDictEqual(
            config.get_bindings("ConfigurableClass"),
            {
                "kwarg1": "okie dokie",
            },
        )
    ...
    """
        ...
        self.assertDictEqual(get_binding_strict("configurable1"), {"kwarg1": 9})
        self.assertDictEqual(get_binding_strict("scope/configurable1"), {})
        self.assertDictEqual(
            get_binding_strict("scope/scope2/configurable1"), {"kwarg1": 7}
        )
        self.assertDictEqual(get_binding_strict("scope2/configurable1"), {})
    ...
    """
        ...
        config.parse_config(config_str)
        non_kwarg = config.get_bindings("configurable1", resolve_references=False)[
            "non_kwarg"
        ]
    ...
    def testGetConfigurable(self):
        ...
        self.assertIs(config.get_configurable(pass_through.__wrapped__), pass_through)
        self.assertIs(config.get_configurable("pass_through"), pass_through)
    ...
    def testAppendAssignment(self):
        ...
        self.assertDictEqual(
            ConfigurableClass().kwarg1,
            {"param1": "param1", "param2": "param2", "param3": 123, "param4": 456},
        )
    ...
    def testQueryMacroParameters(self):
        config_str = """
        MC_VAL1 = 1
        MC_VAL2 = 2
        OTHER_MACRO = 3
        """
        config.parse_config(config_str)
        vars = config.query_all_macro_parameters_with_name_prefix("MC_VAL")
        self.assertDictEqual({"MC_VAL1": 1, "MC_VAL2": 2}, vars)
